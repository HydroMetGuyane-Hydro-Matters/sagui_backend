...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('sagui', '0007_alter_minibasinsdata_options'),
    ]
...
--up-to-date median definition (for postgresql 14)
...
CREATE OR REPLACE FUNCTION guyane.surrounding_days_of_year(c_date date, nbdays int default 10)
...
COMMENT ON FUNCTION guyane.surrounding_days_of_year(c_date date, nbdays int)
...
-- - nbdays: the number of days to retain, around the c_date
-- RETURNS: a 1-column table, listing the days (Day of the year (1-366))
CREATE OR REPLACE FUNCTION guyane.surrounding_days_over_previous_years(c_date date, nbdays int default 10)
RETURNS TABLE (s_dates   date)
AS
$func$
BEGIN
   RETURN QUERY
        SELECT "date" FROM (SELECT DISTINCT "date" FROM guyane.hyfaa_data_mgbstandard) d
        WHERE "date" < (c_date - (nbdays || ' days')::interval)
        AND date_part('doy', "date") IN (
            SELECT guyane.surrounding_days_of_year(c_date, nbdays)
        )
        GROUP BY "date"
        ORDER BY "date" DESC;
   END
$func$  LANGUAGE plpgsql;
COMMENT ON FUNCTION guyane.surrounding_days_over_previous_years(c_date date, nbdays int)
